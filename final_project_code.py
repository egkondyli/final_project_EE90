...
#
# Addresses for DP0-DP2 carried over from the previous board.
DIGITAL_POTS = {
    ...
    6:  {"address": 0b0101110, "max_resistance": R_10K},
    7:  {"address": 0b0101100, "max_resistance": R_100K},
...
def set_cap_bank(bank, sw0, sw1, sw2):
    ...
    INPUTS
        bank: 2 -> Wein cap bank (real values from schematic)
              1 -> other bank (PLACEHOLDER values, see CAP_BANKS)
    ...
    NOTE: this assumes both banks share the same SW_0/1/2 lines and only
    differ in capacitor values. If Bank 1 has its own switch pins, this
    needs separate pin control.
    """
